...
def scheduling():
    ...
    for i in range(len(dirty)):
        fr = ir2fr(psf[i], sky[i].shape)
        quad_freq.append(wiener(dirty[i], np.zeros_like(sky[i]), fr, lamb))
...
def show_courbes():
    ...
    for i in range(len(dirty)):
        ...
        plt.plot((quad_freq[i])[800], label='quad')
...
def show_images():
    ...
    for i in range(len(dirty)):
        plt.figure(i+1)
        plt.clf()
        plt.imshow(quad_freq[i], vmin = np.min(quad_freq[i]), vmax = np.max(quad_freq[i]))
